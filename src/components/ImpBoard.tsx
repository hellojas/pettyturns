        ...
        return (
          ...
          >
            ...
            {isAlliance ? (
              ...
              <span className="text-[7px] leading-none font-bold" style={{ color: '#f7ecd755' }}>{level}</span>
            )}
          </div>
        );
      })}
...
function SurfaceBand({
  ...
  return (
    <div className="relative overflow-hidden p-2.5" style={{ background: tint }}>
      ...
      <div className="relative grid gap-1.5" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
        {spaces.map((s) => (
          <SpaceTile key={s.id} space={s} view={view} legal={legalTargets.has(s.id)} selected={selectedSpace === s.id} onSelect={() => onSelect(s.id)} />
        ))}
      </div>
